...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        XX = np.tile(np.diag(X.dot(X.T)), (num_train, 1)).T
        YY = np.tile(np.diag(self.X_train.dot(self.X_train.T)), (num_test, 1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            dists_i = dists[i, :]
            k_near_idx = np.argsort(dists_i)[:k]
            closest_y = self.y_train[k_near_idx]
